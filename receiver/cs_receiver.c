...
#if USE_FIFO
  const char *ffplay_command[] = {"ffplay", "-framerate", "50", "-infbuf", "-framedrop", "-analyzeduration", "1", FIFO_PATH, NULL};
  const char *location_buf = "location="FIFO_PATH;
#else
  const char *ffplay_command[] = {"ffplay", "-framerate", "50", "-infbuf", "-framedrop", "-analyzeduration", "1", "-", NULL};
#endif
...
int main(int argc, char* argv[]) {
  ...
  for (;;) {
    ...
    // Mux tcp socket if client mode is enabled
    ...
    int bits;
    if((bits = select(max_sock + 1, &fd_r, NULL, NULL, &tv)) == -1) {
      ...
      if (tcp_client_sock < 0) {
        ...
        if (tcp_client_sock < 0) {
          ...
          if (!strncmp(gst_sink, "ffplay", 6)) {
            gst_pid = popen2(ffplay_command, &gst_in_fp, &gst_out_fp);
          } else {
#if USE_FIFO
#ifdef VPUDEC
